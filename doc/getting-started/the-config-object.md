...
Blus takes a configuration objetct. In this object there can be many properties applied.   \
...
The most important property is the Steps property. This property must be a table of so called Step Configs. A Step in Blus describes a single transformation applied to your script by the Blus obfuscation pipeline. A StepConfiguration consists of the Name of the Step as well as settings for the step. All Steps will later be applied in the order they are defined. A single Step can be defined twice and will then be applied twice.
